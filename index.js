...
function handleDrag($el, storageKey) {
    ...
    const onStart = (e) => {
        if ($(e.target).hasClass('moodtube-ctrl') || $(e.target).is('input')) return;
...
async function initializeExtension() {
    ...
    $(`<style>
        ...
        #moodtube-mini-player { transition: width 0.3s, height 0.3s, border-radius 0.3s, padding 0.3s; }
        
        /* Compact Mode Overrides */
        #moodtube-mini-player.moodtube-compact {
        ...
        #moodtube-mini-player.moodtube-compact #moodtube-cover-container {
            width: 50px !important;
            height: 50px !important;
            border-width: 2px !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-cover-hole {
            width: 8px !important;
            height: 8px !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-title-container {
            align-items: flex-start !important;
            text-align: left !important;
            flex: 1 !important;
            min-width: 0;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-controls-container {
            margin-top: 0 !important;
            gap: 15px !important;
            margin-right: 50px !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-btn-playpause {
            font-size: 22px !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-btn-stop,
        #moodtube-mini-player.moodtube-compact #moodtube-btn-ai {
            font-size: 16px !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-volume-container {
            display: none !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-btn-resize {
            top: 50% !important;
            transform: translateY(-50%) !important;
            right: 35px !important;
        }
        #moodtube-mini-player.moodtube-compact #moodtube-btn-close {
            top: 50% !important;
            transform: translateY(-50%) !important;
    ...
    if ($('#moodtube-mini-player').length === 0) {
        let isCompact = localStorage.getItem('moodtube_compact') === 'true';
        let resizeIcon = isCompact ? 'fa-expand' : 'fa-compress';
        
        $(`
        <div id="moodtube-mini-player" class="${isCompact ? 'moodtube-compact' : ''}" style="
            position: fixed; top: 150px; left: 50%; transform: translateX(-50%);
            background: ${BG_COLOR}; border: 1px solid rgba(141, 183, 213, 0.3);
            border-radius: 25px; padding: 25px 20px; color: #fff;
            font-family: -apple-system, sans-serif;
            z-index: 9998; display: none; flex-direction: column; align-items: center; gap: 15px;
            box-shadow: 0 15px 35px rgba(0,0,0,0.8), inset 0 0 10px rgba(141, 183, 213, 0.1); 
            width: 250px; ${BLUR_CSS} cursor: grab; user-select: none;
        ">
            <div id="moodtube-cover-container" style="width: 140px; height: 140px; border-radius: 50%; background: #050505; border: 3px solid ${ACCENT_COLOR}; box-shadow: 0 5px 15px rgba(0,0,0,0.7); display: flex; justify-content: center; align-items: center; position: relative; overflow: hidden; flex-shrink: 0;">
                ...
                <div id="moodtube-cover-hole" style="position: absolute; width: 14px; height: 14px; background: #222; border-radius: 50%; border: 1px solid ${ACCENT_COLOR};"></div>
            </div>
            
            <div id="moodtube-title-container" style="display: flex; flex-direction: column; align-items: center; width: 100%; text-align: center;">
            ...
            <div id="moodtube-controls-container" style="display: flex; gap: 20px; align-items: center; margin-top: 5px; flex-shrink: 0;">
            ...
            <div id="moodtube-volume-container" style="display: flex; align-items: center; width: 90%; margin-top: 10px;">
            ...
            <i class="fa-solid ${resizeIcon} moodtube-ctrl" id="moodtube-btn-resize" style="position: absolute; top: 15px; right: 45px; cursor: pointer; color: #888; font-size: 16px;" title="Compact Mode"></i>
            <i class="fa-solid fa-xmark moodtube-ctrl" id="moodtube-btn-close" style="position: absolute; top: 15px; right: 20px; cursor: pointer; color: #888; font-size: 18px;" title="Close"></i>
        </div>
        `).appendTo('body');
        ...
        $('#moodtube-btn-resize').on('click', () => {
            const $player = $('#moodtube-mini-player');
            const willBeCompact = !$player.hasClass('moodtube-compact');
            $player.toggleClass('moodtube-compact', willBeCompact);
            localStorage.setItem('moodtube_compact', willBeCompact);
            $('#moodtube-btn-resize').attr('class', willBeCompact ? 'fa-solid fa-expand moodtube-ctrl' : 'fa-solid fa-compress moodtube-ctrl');
        });
